...
class Enhancer:
    ...
    def enhance_cuts(
        ...
        def _save_worker(orig_cuts, x_hat, recording_id, speaker, snrs, target_speaker_idx,):
            ...
            for cut in orig_cuts:
                ...
                mixture_paths = {}
                snrs_dict = {}
                for i, ch in enumerate(cut.channel):
                    mixture_info = cut.recording.sources[ch]
                    assert ch == mixture_info.channels[0], (ch, mixture_info[0])
                    mixture_paths[int(ch)] = mixture_info.source
                    snrs_dict[int(ch)] = float(snrs[i])
                # sort snrs
                snrs_dict = dict(sorted(snrs_dict.items(), key=lambda x: x[1], reverse=True))
                # make metadata
                metadata = {
                    "gss_reference_channels": list(snrs_dict.keys()),
                    "mixture_paths": mixture_paths,
                    "snrs": snrs_dict,
                    "gss_output_path": str(out_dir / save_path),
    ...
    def enhance_batch(
        ...
        for i in range(num_chunks):
            st = i * chunk_size
            en = min(T, (i + 1) * chunk_size)
            X_hat_chunk, snrs = self.bf_block(
                Obs[:, st:en, :],
