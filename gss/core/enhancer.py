...
class Enhancer2:
    ...
    from gss.beamformer.souden_mvdr import beamform_mvdr2
    bf_block2 = beamform_mvdr2
    ...
    def wpe_istft(self, X):
        from gss.core.stft_module import istft
        # from gss.core.stft_module import istft_cupy as istft
    ...
    def enhance_cuts(
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            for batch_idx, batch in enumerate(dl):
                batch = SimpleNamespace(**batch)
                logging.info(
                    f"PProcessing batch {batch_idx+1} {batch.recording_id, batch.speaker}: "
                    f"{len(batch.orig_cuts)} segments = {batch.duration}s (total: {total_processed} segments)"
                )
                total_processed += len(batch.orig_cuts)
    ...
    def enhance_batch(
        ...
        for i in range(num_chunks):
            ...
            if self.wpe_block is not None:
                ...
                Obs[:, st:en, :] = Obs_chunk
        # GSS
        Obs = self.gss_stft(cp.asarray(self.wpe_istft(Obs))).astype(Original_dtype)
        D, T, F = Obs.shape
        chunk_size = int(np.ceil(T / num_chunks))
        for i in range(num_chunks):
            st = i * chunk_size
        ...
        X = Obs * target_mask[..., None, :, :]
        N = Obs * distortion_mask[..., None, :, :]
        X = self.mvdr_stft(cp.asarray(self.gss_istft(X))).astype(Original_dtype)
        N = self.mvdr_stft(cp.asarray(self.gss_istft(N))).astype(Original_dtype)
        Obs = self.mvdr_stft(cp.asarray(self.gss_istft(Obs))).astype(Original_dtype)
        ...
        X_hat = cp.concatenate(X_hat, axis=0).transpose()

        logging.debug("Computing inverse STFT")
        x_hat = self.mvdr_istft(X_hat)  # returns a numpy array
        if x_hat.ndim == 1:
            x_hat = x_hat[np.newaxis, :]
